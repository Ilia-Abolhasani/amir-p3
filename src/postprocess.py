...
def filter2_run(inp, config):
    ...
    level2 = level2[
        level2["hit complementarity percentage"]
    ...
    level2 = level2[
        level2["num of linking residues"] >= config.num_of_linking_residues_min
    ]
    level2 = level2[
        level2["num of linking residues"] <= config.num_of_linking_residues_max
    ]

    level2 = level2[level2["hit GC content"] >=
    ...
    cols = ["mismatch type", "mismatch size"]
    sum_missmatch = level2[cols].apply(
        lambda row: _sum_of_size_in_hit(row, "mismatch type", "mismatch size"), axis=1
    )

    cols = ["bulge type", "bulge size"]
    sum_bulge = level2[cols].apply(
        lambda row: _sum_of_size_in_hit(row, "bulge type", "bulge size"), axis=1
    )

    cols = ["internal type", "internal loop total size"]
    sum_internal = level2[cols].apply(
        lambda row: _sum_of_size_in_hit(
    ...
    sum_internal_hsbl = level2[cols].apply(
        lambda row: _sum_of_size_in_hit(
            row, "internal type", "internal loop HSBL"),
        axis=1,
    )
    ...
    cols = [titles.mir_type, "bulge type",
            "bulge size", "bulge start", "bulge end"]
    ...
    sum_bulge_zero = level2[cols].apply(
        lambda row: _sum_of_size_in_hit_only_zero(row), axis=1
    )

    cols = [titles.hit_start, titles.hit_end, titles.psep, titles.mir_type]
    sum_of_residue = level2[cols].apply(
        lambda row: _number_of_residue(row), axis=1)
