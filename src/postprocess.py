...
def _postprocess_run(inp, config):
    ...
    if ("max_allowed_bulge_size_in_hit_region" in config):
        level2 = level2[
    ...
    if ("max_allowed_internal_loop_size_in_hit_region" in config):
        level2 = level2[
    ...
    if ("minimum_required_clear_region" in config):
        level2 = level2[
    ...
    if ("border_line_mismatch_max" in config):
        level2 = level2[
            level2[["mismatch type", "mismatch size"]].apply(
    ...
    if ("border_line_bulge_max" in config):
        level2 = level2[
            level2[["bulge type", "bulge size"]].apply(
                lambda row: _check_border_line(
    ...
    if ("border_line_internal_max" in config):
        level2 = level2[
            level2[["internal type", "internal loop total size"]].apply(
                lambda row: _check_border_line(
                    row,
    ...
    if ("max_allowed_hsbl_ssbl_size" in config):
        level2 = level2[
    ...
    if ("acceptable_num_for_hit_locations_in_bulges_or_loops" in config):
        level2 = level2[level2[titles.accept_num_hit_loc_bulg_loop] <=
                        config.acceptable_num_for_hit_locations_in_bulges_or_loops]
    if ("acceptable_num_for_unmatched_locations_in_hit_region" in config):
    ...
    if ("border_line_structure_allowance" in config):
