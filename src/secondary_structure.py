...
def _viennarna(result_path, num_cpus):
    ...
    os.system('RNAfold --jobs=0 --infile {current_path}/{temp_path}/extended_modified.txt  --noPS -T 22 > {current_path}/{base}/viennarna_result.txt')
...
def _contrafold(result_path, num_cpus):
    ...
    def run_contrafold(tag):
        ...
        os.system('./contrafold predict ../..{base[1:]}{tag}/{tag}.FASTA > ../..{base[1:]}{tag}/{tag}.dot')
        ...
        with open(f"../..{base[1:]}{tag}/{tag}.dot", 'w') as file:
            file.write('\n'.join(text[1:]))    
        os.system('RNAeval  ../..{base[1:]}{tag}/{tag}.dot -T 20 > ../..{base[1:]}{tag}/{tag}.dotdg    ')
