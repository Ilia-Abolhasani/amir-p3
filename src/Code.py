...
def get_mir_type(hit_start, hit_end, istar_min, istar_max, continuous_pairing, complementarity_in_hit_region, hit_self_complementarity):
    ...
    print(hit_start, hit_end, istar_min, istar_max, continuous_pairing,
          complementarity_in_hit_region, hit_self_complementarity)
...
def get_mismatch(domain, values, MCMA, hit_start, hit_end, mir_type):
    size = []
    location_type = []
    location_start = []
    location_end = []
...
# MCMA: maximum consecutive mismatch allowance
def get_internal_loop(domain, values, MCMA, hit_start, hit_end, mir_type):
    size_HSBL = []  # number of
    size_SSBL = []
    location_type = []
    location_start = []
...
def get_trim_data(nucleotide, index, values, start, end):
    ...
    _v = values.copy()[start-1:end].apply(lambda x: 0 if x ==
                                          0 else max(x - (start - 1), 0)).reset_index(drop=True)
    return [_n, _i, _v]
...
def get_dg_by_vienna(dotbracket, write_path):
    ...
    process = Popen(["RNAeval", f"{write_path}.dot", "-T", "22"], stdout=PIPE)
    (output, err) = process.communicate()
    exit_code = process.wait()
    os.remove(f'{write_path}.dot')
    if(err):
        dg = None
    else:
...
def get_row(tag, path, extra, acceptable_terminal_structures=5, MCMA=2, effective_bulge_size_in_Hit_vicinity_regions=4,
    ...
    result['full seq visualization'] = visualization(
        nucleotide, index, values, hit_start, hit_end, None, None, None, None)
    ...
    [complementarity_in_hit_region,
        complementarity_in_hit_region_percentage] = get_complementarity_in_hit_region(inc_srange, hit_len)
    ...
    continuous_pairing = get_continuous_pairing(
        hit_start, hit_end, flanking_istar_min, flanking_istar_max, hit_self_complementarity)
    ...
    mir_type = get_mir_type(hit_start, hit_end, istar_min, istar_max,
                            continuous_pairing, complementarity_in_hit_region, hit_self_complementarity)
    ...
    result['full seq visualization'] = visualization(
        nucleotide, index, values, hit_start, hit_end, boi_start,  boi_end, star_start_real, star_end_real)
    ...
    if(energy_calc_method == "Vienna" or energy_calc_method == "UNAFold"):
        ...
        result['boi visualization'] = visualization(_n, _i, _v, hit_start - (boi_start - 1), hit_end - (
            boi_start - 1), 1,  boi_end - (boi_start - 1), star_start_real - (boi_start - 1), star_end_real - (boi_start - 1))
    ...
    result['precursor seq visualization'] = visualization(_n, _i, _v, hit_start - (
        s - 1), hit_end - (s - 1), 1,  e - (s - 1), star_start_real - (s - 1), star_end_real - (s - 1))
    result['terminal structure range'] = [
    ...
    if(len(terminal_structure_range) == 0):
        ...
        if(number_of_terminal_structure == 0):
            ...
            result['number of terminal structures'] = 1
            [branch_start_point, branch_end_point] = [
                [terminal_structure_range[0]+1], [terminal_structure_range[-1]+1]]
            stem_last_residue = []
        else:
        ...
        if(number_of_terminal_structure != 0):
            ...
            [branch_apical_loop_start, branch_apical_loop_end, branch_apical_loop_size] = get_branch_apical_loop_size(
        ...
        for i in range(acceptable_terminal_structures):
            if(i < len(branch_start_point)):
                ...
                result[f'branch#{i + 1} total length'] = abs(
                    branch_end_point[i] - branch_start_point[i]) + 1
        ...
        primary_stem_end_point = get_primary_stem_end_point(
            branch_start_point, branch_end_point, stem_last_residue, hit_start, hit_end, istar_min, istar_max, values, number_of_terminal_structure, mir_type)
        if(not np.isnan(primary_stem_end_point)):
            ...
            if(number_of_terminal_structure == 0):
                ...
                result[f'branch#{1} apical loop size'] = primary_stem_end_point_star - \
                    primary_stem_end_point - 1
                result[f'branch#{1} stem length'] = 0
            primary_stem_length = get_primary_stem_length(
                primary_stem_end_point, branch_start_point, branch_end_point, stem_last_residue, hit_start, hit_end, values, number_of_terminal_structure, mir_type)
            result['primary stem length'] = primary_stem_length

            domain = get_domain(primary_stem_end_point, boi_start,
                                boi_end, stem_last_residue, hit_start, hit_end, mir_type)
            result['domain'] = [domain[0]+1, domain[-1]+1]
            domain_star = get_domain_star(
                primary_stem_end_point_star, boi_start, boi_end, stem_last_residue, hit_start, hit_end, mir_type)
            ...
            [mismatch, mismatch_size, mis_loc_type, mis_start, mis_end] = get_mismatch(
            ...
            [bulge, bulge_size, bulge_loc_type, bulge_start, bulge_end,
                bulge_type] = get_bulge(domain, values, hit_start, hit_end, mir_type)
            result['bulge'] = bulge
            result['bulge size'] = bulge_size
            result['bulge type'] = bulge_loc_type
            result['bulge start'] = bulge_start
            result['bulge end'] = bulge_end
            [internal_loop, size_HSBL, size_SSBL, intr_loc_type, intr_start,
                intr_end] = get_internal_loop(domain, values, MCMA, hit_start, hit_end, mir_type)
            ...
            [proximal, proximal_hr, proximal_counter] = get_distance_info("loop proximal", "proximal border line", mis_loc_type, mismatch_size, mis_start,
                                                                          bulge_size, bulge_loc_type, bulge_start, bulge_end, bulge_type, internal_loop, size_HSBL, size_SSBL, intr_loc_type, intr_start, intr_end, mir_type)
            result['proximal distance'] = proximal_hr
            [distal, distal_hr, distal_counter] = get_distance_info("loop distal", "distal border line", mis_loc_type, mismatch_size, mis_start, bulge_size,
                                                                    bulge_loc_type, bulge_start, bulge_end, bulge_type, internal_loop, size_HSBL, size_SSBL, intr_loc_type, intr_start, intr_end, mir_type)
            ...
            result['Loop distal junction distance'] = get_junction_distance(
                distal, boi_dist, effective_bulge_size_in_Hit_vicinity_regions, effective_internal_loop_size_in_Hit_vicinity_regions)
            result['Loop proximal junction distance'] = get_junction_distance(
                proximal, psep_dist, effective_bulge_size_in_Hit_vicinity_regions, effective_internal_loop_size_in_Hit_vicinity_regions)
        else:
...
d = get_df_by_tag("CM040440.1|-|10011655-10012075|201-221", extra=0)
print(d.shape)
